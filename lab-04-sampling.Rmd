...
df$canopy <- c("NO DATA, but these columns were in the self-check section")
df$climate <- c("NO DATA, but these columns were in the self-check section")
...
> *Daily has more variability since there is only data for January so I cannot compare months.  The variable is about carbon flux and the average is already close to zero, the difference is small. Although, I think carbon is more unacceptable to change between seasons with all the vegetation change than it is on a day to day basis. This can't be proven without more data that I don't have though..*
...
reps <- 100   # TODO: choose number of replicates (e.g., 500 or 1000)
...
> *Yes, the sampling sitrobutoin appears to be centered on the population mean. Nothing seems to change when increasing/deacreasing n_samp, other than the ammount of time rstudio takes to output the graph.*
...
> *Systematic sampling approximated the population mean better, or more accurately, than SRS. However, both SRS and stratification both provided a more accurate approximation of SD than systematic sampling. Changing the K value seems to have no effect on the values. Both the daily and seasonal changes in the carbon cycle would be interacting with the k value.  *
...
> *Cluster sampling groups the data into clusters, which is often less accurate than giving everything an equal chance to be chosen for sampling. Since the cluster is grouping the data on a temporal scale of days, time series can come into play. Autocorrelation can be used to evaluate temporal dependency. *
...
> *Yes, quasi-continous sampling preserves the seasonal and daily patterns. Since you can adjust your step size/how often  a sample is recorded, the data's temporal resolution is completely up to the researcher. In an EAES context, quasi-continuous sampling can be done autonomously at a relatively low cost with minimal human activity required, while discrete sampling, even when sparse, can be time intensive. One example of quasi-continuous sampling that comes to mind is the data collected by the sapflow sensors around UIC.*
...
> *Systematic and Quasi-continuous sampling both gave very accurate estimated of the population mean and SD. However, systematic sampling was slightly more accurate than the quasi-continuous estimate, this difference is small enough that it could be considered negligible in most scenarios. Since there was only one month in the data set, the seasonal sampling could not preform well, but it makes sense that systematic and quasi-continuous sampling measured the diurnal patterns well. They can take samples from whatever temporal resolution is necessary. For an EAES study with limited field days, I would try to make a systematic sampling plan beforehand, and then place sensors in the predetermined locations that are cable of taking remote quasi-continous samples. This way you could get the benefits of both methods, and while the sensors may be expensive, the cost after they are deployed should be minimal.*
